...
type Config struct {
	...
	// producer default is 1 (serial encode). Calibrate to the host
	// pod's CPU limit for CPU-bound workloads.
	...
	// default is 1 (one batch per CAS); raising it (e.g. to 16)
	// enables coalescing, which trades a small batch-level latency
	// increase for ~N× lower per-batch
	...
	// p99.5 of `opendataexporter_durable_wait_duration_seconds` runs
	// 2-3s under load, with bursts past 5s when downstream merge
	// pressure spikes. Configure explicitly per deployment. A value of 0
