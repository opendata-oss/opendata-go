...
// TestDefaultProducerConfig_PreservesPrePhase3Behavior asserts that
// `DefaultProducerConfig` returns Phase-3 pipelining knobs at values
// that reproduce the single-flight pre-Phase-3 behavior. The Phase-3
// run-loop rewrite must compile and behave identically against this
// config until operators explicitly opt into pipelining.
//
// See `plans/odb-high-throughput/phase03-producer-pipelining-design.md`
// §Defaults for the policy this test pins.
func TestDefaultProducerConfig_PreservesPrePhase3Behavior(t *testing.T) {
	cfg := DefaultProducerConfig()

	// Pre-existing fields, unchanged by Phase 3.2.
	...
	// Phase-3 pipelining defaults: must reproduce single-flight behavior.
