...
// producerBudget enforces the in-flight backpressure caps:
// `MaxInFlightBytes` (the binding memory bound) and
// `MaxInFlightBatches` (a secondary count safety).
//
...
type producerBudget struct {
	...
	// terminated (committed or failed).
	drainCh chan struct{}
}
...
// or ErrShutdown if those fire first. Backs Flush(ctx), which blocks
...
// per-batch footer). The reservation is reconciled to the
// accumulator's actual byte count (entries + metadata + buffer
// framing). May transiently exceed `MaxInFlightBytes` by up to
// ~`MaxInFlightBatches * (4 * entries + 7)` bytes — bounded and small
// relative to the cap.
