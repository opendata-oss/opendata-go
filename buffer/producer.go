...
type batchAccumulator struct {
	...
	// the budget on terminal outcome.
	byteCost  int64
	startedAt time.Time
	started   bool
}
...
// `oldestUnflushed` clock that the
...
// pool. Encoders consume `pendingBatch` and produce `encodedBatch`
// after running EncodeBatch.
//
// The post-rotator pipeline is Rotator → Encoder pool → Uploader →
// ManifestCommitter → WatcherResolver, each stage a separate goroutine
// (or pool) connected by channels.
...
// (computed from runID + ordinal), and a `pipelineStartedAt`
...
type Producer struct {
	...
	// collide on a path even if they pick the same ordinals.
	runID string
	...
	// blocks on send until the Encoder picks the batch up, giving
	// single-flight throughput. With higher EncodeConcurrency (opt-in),
	// one of the N encoders picks up each batch as workers free up.
	...
	// would each trigger their own CAS regardless of batch size
	// (e.g. 32 ready ordinals coalesce into 2 CAS calls).
	...
	// waiting for AwaitDurable consumers.
	...
	// budget enforces MaxInFlightBytes and MaxInFlightBatches.
	// Reservations are made in AppendContext (bytes) and the rotator
	// emit (batch slot); both released by the WatcherResolver on
	// terminal outcome.
	budget *producerBudget

	// Metrics state (atomic counters for `workers_busy`).
	...
	// as `producer_oldest_unflushed_batch_age_seconds`.
	...
	// failure across ordinals — without this seam, the
	// "encode failure advances committer past missing ordinal" path
	// can only be tested with all-bad batches, which doesn't
	// distinguish advancing past one bad ordinal from bypassing all of
	// them. Production code must not set this.
...
// the committer as a synthetic completion. The resolver calls
// OnFlush, resolves all watchers with `err`, and signals flushAck.
//
// This is split out of the ManifestCommitter so the committer can
// move on to the next CAS without waiting for AwaitDurable consumers
// to drain.
type resolverItem struct {
	...
	// `buffer.producer.batch_outcome` counter.
...
func NewProducer(store objstore.ObjectStore, config ProducerConfig) *Producer {
	// Apply defaults for the pipelining fields that may be unset when
	// callers build a ProducerConfig literally (e.g. test code). This
	// keeps the budget + retry budgets meaningful even for partial
	// configs while still letting callers opt into tighter caps
	// explicitly. Production code should call DefaultProducerConfig()
	// and override.
	//
	// Without these defaults, a ProducerConfig literal that omits
	// UploadMaxAttempts / ManifestMaxAttempts would fall through to the
	// local "unset → 1" fallback in putWithRetry / manifestCommitter,
	// meaning no retry budget at all.
	...
	// stages have already drained. The committer is the single
	// resolver sender (there is no encoder→resolver direct path).
	go func() {
		<-p.committerDone
		close(p.resolverCh)
	}()
...
// (no other goroutine increments the counter).
//
...
func (p *Producer) rotator() {
	...
	emit := func(reason FlushReason, flushAck chan error) {
		if batch.isEmpty() {
			...
			// `Flush` returns nil for an empty buffer.
			if flushAck != nil {
				flushAck <- nil
			}
			return
		...
		// Batch slot acquired — emit the inflight gauges so
		// `buffer.producer.inflight_batches` reflects the current
		// state, not just the post-release state.
		p.emitInflightSnapshot()
		stats := batch.stats()
		...
		// footer (batchFooterSize bytes). The reservation is
		// reconciled to the accumulator's actual byte count (entries +
		// metadata + buffer framing). Without this, many tiny records
		// undercount the budget and may blow MaxInFlightBytes during
		// encoding.
		...
		// propagates through appendCh to Append callers — giving
		// single-goroutine behavior at default concurrency.
		p.pendingCh <- pb
...
// encodedCh when all of them have exited.
//
...
// encodedBatch values to encodedCh.
//
...
func (p *Producer) encoder() {
	for pb := range p.pendingCh {
		...
		// per-batch marshaled bytes (hundreds of MiB at typical
		// batch sizes).
		pb.entries = nil
		busy = p.encodeWorkersBusy.Add(-1)
		if p.config.Observer != nil {
...
// closes uploadCompletionCh when all of them have exited.
//
...
// uploadCompletions map; the out-of-order-upload-completion test in
// `producer_test.go` is the dedicated check for this reordering
// invariant.
...
// (retried up to UploadMaxAttempts with exponential backoff), and
// signals the result to the ManifestCommitter via
// uploadCompletionCh. PUT success and PUT
...
// error.
//
// Retry strategy: retry every error uniformly until UploadMaxAttempts
// is hit. Ideally retryable (network/5xx/throttling) errors would be
// distinguished from non-retryable (4xx other than 429), but the
// objstore package does not categorize errors, so we retry
// uniformly. Non-retryable failures pay a
...
func (p *Producer) uploaderWorker() {
	for eb := range p.encodedCh {
		...
		// per in-flight slot (~6 MiB × MaxInFlightBatches at typical
		// batch sizes, in the hundreds of MiB at the peak). Size and
		// location are already captured in the encodedBatch fields.
		eb.payload = nil
...
// Single goroutine — manifest mutation is serialized through one
// owner.
//
...
// UploadConcurrency=1 (the default) completions arrive in order, so
// the map holds at most one item at a time.
//
...
// CAS group.
//
// Coalescing: at ManifestAppendBatchSize=1 (the default), every
// contiguous run of length 1 results in one CAS. The structure
// (multi-item enqueueBatch + per-group watcher resolution + flushAck
// signaling) is exercised even at size=1.
func (p *Producer) manifestCommitter() {
	...
	// upstream goroutines don't get stuck.
	halted := false
	...
	// resolve hands off the batch's outcome to the WatcherResolver so
	// the committer can return to its loop and start the next CAS
	// without waiting for AwaitDurable consumers.
	...
	// the cumulative conflict count + the final error.
	...
	// of batch size, defeating coalescing.
	//
	...
	for {
		...
		if closed {
			...
			// and their watchers were never resolved.
			return
		}
	}
}
...
// Flush-triggered batches) signaling flushAck.
//
// Single goroutine. Keeping this separate from the ManifestCommitter
// frees the committer to start its next CAS as soon as the previous
// CAS returns, without waiting for AwaitDurable consumers to drain.
...
// framed-byte reconciliation, and resolver release — so the
// "current in-flight" gauges (`buffer.producer.inflight_bytes` /
// `inflight_batches`) actually reflect the current state. Without
// this, a stuck upload would never produce a positive gauge sample,
// then emit 0 on release.
...
// AppendContext is the cancellation-aware variant of Append.
// Callers that don't need cancellation stay on the context-free
// Append; callers that want to bound the
...
// bytes against `MaxInFlightBytes` *before* the appendCh send. The
// reservation is released by the
...
func (p *Producer) AppendContext(ctx context.Context, entries [][]byte, metadata []byte) (*WriteHandle, error) {
	...
	// Bytes reserved — emit the inflight gauges so
...
// in-flight batch has either committed or failed. Forces a final
// manifest CAS even if the current commit-pending queue is below
// ManifestAppendBatchSize.
//
// Two-phase wait:
...
// flush). An OTel collector that embeds this producer publishes this
// as the Prometheus gauge `producer_oldest_unflushed_batch_age_seconds`
// for lag accounting.
//
...
// Detecting pipeline-stage stalls beyond the rotator is future work;
// this metric is sufficient for catching the "rotator is wedged on
// appendCh while records pile up" signal.
